...
function getDownlinesRecursive(parentUid) {
    ...
    children.forEach(child => {
        list.push(child); list = list.concat(getDownlinesRecursive(child.uid));
    });
    return list;
}
...
// ==========================================
// 5. DASHBOARD (UPDATE LOGIKA PERINGKAT)
// ==========================================
function renderDashboard() {
    ...
    // 2. Total & Peringkat
    const totalTeam = myTeamData.length - 1; // Kurangi 1 (diri sendiri tidak dihitung utk syarat tim?)
    // *Revisi:* Biasanya "Total Anggota Tim" menghitung downline saja.
    // Di kode sebelumnya myTeamData isinya [Saya, Anak, Cucu].
    // Jadi Total Downline = myTeamData.length - 1.
    ...
    const countPrev = myTeamData.filter(x => x.joinDate <= prevEnd).length; // Hitung prev termasuk saya?
    // Agar konsisten, target biasanya dari total volume. Kita pakai total volume (termasuk saya) atau downline?
    // Mari kita konsisten pakai myTeamData.length (Total Volume Tim).
...
function calculateRank(total) {
    ...
    if (nextRankIndex >= 0) {
        ...
        document.getElementById('rankNextGoal').innerText = "Top Level Tercapai!";
        document.querySelector('.total-right .next-desc').innerText = "Selamat!";
    }
}

// ==========================================
// 6. LIST & NETWORK (TETAP SAMA)
// ==========================================
function initList() {
    window.sortData = (col) => {
...
function renderTable() {
    ...
    sorted.forEach((m, i) => {
        ...
        html += `<tr><td class="col-no">${i + 1}</td><td>${m.name}</td><td class="col-uid">${m.uid}</td><td class="col-ref">${refUid}</td><td class="col-date">${dateStr}</td></tr>`;
    });
