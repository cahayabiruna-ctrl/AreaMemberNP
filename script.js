...
// ==========================================
// 5. DASHBOARD (LOGIKA PERIODE DIPERBAIKI)
// ==========================================
function renderDashboard() {
    ...
    let isPeriod2 = false; // Penanda apakah kita sedang di P2

    if (d === 31) { 
        // Tgl 31 dianggap awal bulan depan
        ...
        prevEnd = new Date(y, m, 0, 23, 59, 59); // Akhir bulan lalu
    ...
    // 5. Chart (Warna Dinamis)
    renderChart(myFullTeam, y, m, isPeriod2);
}

function renderChart(teamData, y, m, isPeriod2) {
    const ctx = document.getElementById('growthChart').getContext('2d');
    
    // Set Batas Tanggal
    const p1Start = new Date(y, m, 1);
    const p1End = new Date(y, m, 15, 23, 59, 59);
    
    const p2Start = new Date(y, m, 16);
    const p2End = new Date(y, m, 30, 23, 59, 59); // Mentok tgl 30 (31 masuk P1)

    // Hitung Data
    const c1 = teamData.filter(x => x.joinDate >= p1Start && x.joinDate <= p1End).length;
    const c2 = teamData.filter(x => x.joinDate >= p2Start && x.joinDate <= p2End).length;

    // WARNA DINAMIS: 
    // Jika sekarang Periode 1: P1 Emas, P2 Gelap
    // Jika sekarang Periode 2: P1 Gelap, P2 Emas
    ...
    window.myChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: ['Periode 1 (1-15)', 'Periode 2 (16-30)'],
            datasets: [{
                label: 'Pertumbuhan',
                data: [c1, c2],
...
function calculateRank(total, direct) {
    const ranks = [
    ...
    if (currentRank) {
        ...
        if (idx > 0) {
            ...
            gap = next.min - total;
            nextName = next.name;
            descHtml = `Anggota lagi<br><span style="color:#D4AF37; font-weight:bold;">Menuju ${nextName}</span>`;
        } else {
            gap = 0; descHtml = "Maksimal"; nextName = "Top Level";
        }
    } else {
        if (direct >= 5) {
            rankName = "V.I.P 1";
            gap = 31 - total;
            nextName = "V.I.P 2";
            descHtml = `Anggota Tim lagi<br><span style="color:#D4AF37; font-weight:bold;">Menuju ${nextName}</span>`;
        } else {
            rankName = "MEMBER"; 
            gap = 5 - direct;
            nextName = "V.I.P 1";
            descHtml = `Direct Downline lagi<br><span style="color:#D4AF37; font-weight:bold;">Menuju ${nextName}</span>`;
        }
    }
...
// ==========================================
// 6. FUNGSI LIST & NETWORK (TETAP SAMA)
// ==========================================
function initList() {
    window.sortData = (col) => {
