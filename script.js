...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (path.includes('index.html')) {
        ...
        // JALANKAN DASHBOARD SETELAH DATA SIAP
        renderDashboard();
    } else if (path.includes('list.html')) {
        // SIAPKAN LIST SETELAH DATA SIAP
        prepareMyTeamData(); 
        initList();
    } else if (path.includes('network.html')) {
        // SIAPKAN NETWORK SETELAH DATA SIAP
        prepareMyTeamData();
        initNetwork();
    }
});
...
// ==========================================
// 2. AMBIL DATA (HANYA AMBIL, TIDAK HITUNG)
// ==========================================
async function loadData() {
    try {
        ...
        // Normalisasi Super Aman (Cek Huruf Besar/Kecil)
...
// Fungsi Rekursif Cari Anak Buah
...
// Fungsi Siapkan Data Tim Saya (Dipakai di List & Network)
...
async function doLogin() {
    ...
    await loadData(); // Pastikan data terbaru
...
// ==========================================
// 4. DASHBOARD (HITUNG DI SINI AGAR AKURAT)
// ==========================================
function renderDashboard() {
    const myUid = sessionStorage.getItem('userUid');
    
    // Cari Saya di Data Global yg sudah di-load
    const me = globalData.find(m => m.uid === myUid);
    
    // Jika data belum ada (karena refresh), coba load lagi sebentar
    if(!me && globalData.length === 0) {
...
// LOGIKA PERINGKAT (Update Tampilan Rapi)
function calculateRank(total, direct) {
    ...
    if (currentRank) {
        ...
        if (direct >= 5) {
            ...
            rankName = "MEMBER"; // Ganti MEMBER jadi VIP 0 kalau mau
            gap = 5 - direct;
            nextName = "V.I.P 1";
