...
// ==========================================
// 3. DATA LOGIC (SAYA KEMBALIKAN KE VERSI YG JALAN)
// ==========================================
async function loadData() {
    try {
        ...
        // Normalisasi Data (PENTING: Paksa String agar UID dan Upline cocok)
        globalData = data.map(m => ({
            uid: String(m.UID || m.uid).trim(),
            name: (m.Nama || m.nama || m.name || '-').trim(),
            // Pastikan upline kosong jadi string kosong, bukan null
...
// ==========================================
// 5. DASHBOARD (PERINGKAT & TARGET FIX)
// ==========================================
...
function calculateRank(total) {
    ...
    // Cari Target Selanjutnya
    // Array ranks urut dari Besar ke Kecil. Jadi target selanjutnya adalah index sesudahnya (yang lebih besar min-nya)
    // Logic revisi: array di atas urut dari VIP 9 ke 0.
    // Jika sekarang VIP 3 (index 6), targetnya VIP 4 (index 5).
    
    let currentIndex = ranks.indexOf(currentRank);
    
    if (currentIndex > 0) { // Masih ada level di atasnya
        let nextRank = ranks[currentIndex - 1];
        let need = nextRank.min - total;
        
        document.getElementById('nextLevelGap').innerText = need;
        // Update teks deskripsi di kartu Total
        // Mencari elemen next-desc di dashboard.html
        const descEl = document.querySelector('.next-desc');
        if(descEl) descEl.innerText = `Anggota lagi menuju ${nextRank.name}`;
        
        // Update teks di kartu Peringkat
...
function initNetwork() {
    ...
    diagram.nodeTemplate = $(go.Node, "Horizontal",
        ...
        $(go.Panel, "Auto", { name: "PANEL" },
            ...
            $(go.TextBlock, 
                { 
                    ...
                    stroke: "#fff", font: "11px sans-serif", textAlign: "center",
        ...
        $("TreeExpanderButton", { 
            width: 14, height: 14, alignment: go.Spot.Right, margin: new go.Margin(0, 0, 0, 4), 
            "ButtonBorder.fill": "#222", "ButtonBorder.stroke": "#D4AF37", "ButtonIcon.stroke": "white"
        })
    );
    ...
    const nodes = myTeamData.map(m => {
        ...
        return { 
            ...
            strokeColor: isGold ? "#ffd700" : "#ffffff", 
