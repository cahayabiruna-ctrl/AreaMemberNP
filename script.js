...
// ==========================================
// 7. NETWORK (FINAL: 1 BARIS + EXPAND)
// ==========================================
function initNetwork() {
    ...
    const diagram = $(go.Diagram, "networkDiagram", {
        layout: $(go.TreeLayout, { 
            angle: 0, 
            layerSpacing: 80,  // Jarak antar level
            nodeSpacing: 15    // Jarak antar kotak (Vertikal)
        }),
        "undoManager.isEnabled": true, 
        "initialContentAlignment": go.Spot.Center
    });

    // --- NODE TEMPLATE (KOTAK 1 BARIS + EXPANDER) ---
    diagram.nodeTemplate = $(go.Node, "Horizontal",
        { selectionObjectName: "PANEL" }, // Klik pilih kotaknya
        ...
        $(go.Panel, "Auto", 
            ...
            $(go.Shape, "RoundedRectangle", 
                ...
                // Warna Emas jika Tim >= 5
                new go.Binding("stroke", "strokeColor"),
                new go.Binding("strokeWidth", "strokeWidth")
            ),
            $(go.TextBlock, 
                { 
                    margin: 6, // Padding Ramping
                    stroke: "#fff", 
                    font: "11px sans-serif", 
                    textAlign: "center",
                    maxLines: 1, // KUNCI: CUMA 1 BARIS
                    overflow: go.TextBlock.OverflowEllipsis // Kalau kepanjangan jadi ...
                },
                new go.Binding("text", "label")
            )
        ),

        // TOMBOL EXPAND (BUKA/TUTUP) - Ada di sebelah kanan
        $("TreeExpanderButton",
            { 
                width: 14, height: 14, 
                alignment: go.Spot.Right,
                margin: new go.Margin(0, 0, 0, 5), // Jarak dr kotak
                "ButtonBorder.fill": "#222",
                "ButtonBorder.stroke": "#D4AF37", // Border tombol Emas
                "ButtonIcon.stroke": "white"      // Ikon Putih
            }
        )
    );
    ...
    const nodes = myTeamData.map(m => {
        ...
        return { 
            key: m.uid, 
            // FORMAT SATU BARIS LURUS (DIPISAH GARIS MIRING)
