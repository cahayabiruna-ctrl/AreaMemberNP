...
function initNetwork() {
    ...
    const diagram = $(go.Diagram, "networkDiagram", {
        // TAMBAHKAN PADDING DI SINI (Margin Luas)
        ...
        // Izinkan zoom out lebih jauh
        minScale: 0.1,
        maxScale: 2.0
    });

    // --- NODE TEMPLATE RAMPING ---
    ...
    // FUNGSI DOWNLOAD (High Quality + Margin)
    window.downloadNetworkImage = function() {
        const img = diagram.makeImage({ 
            scale: 2, 
            background: "#000", 
            maxSize: new go.Size(Infinity, Infinity),
            padding: new go.Margin(50) // Padding di hasil download
        });
