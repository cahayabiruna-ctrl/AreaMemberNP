...
function getTotalGroupCount(uid) {
    // Total Group = Diri Sendiri + Downline Recursive
    const downlines = getDownlinesRecursive(uid);
    return 1 + downlines.length;
}
...
function renderDashboard() {
    ...
    // HITUNG STATISTIK VIP TIM (INI YANG KEMARIN HILANG)
...
// LOGIKA MENENTUKAN RANK SESEORANG (Helper untuk statistik)
function getRankLevel(uid) {
    const total = getTotalGroupCount(uid);
    // Cek Direct Downline untuk syarat VIP 1
    ...
    if (direct >= 5) return 1; // Syarat VIP 1: 5 Direct
    return 0; // Member
}

// FUNGSI HITUNG STATISTIK VIP DI TIM SAYA (GRID KANAN)
