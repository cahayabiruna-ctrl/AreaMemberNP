...
function renderDashboard() {
    ...
    // 3. Stats VIP (Masukan Saya + Downline ke kalkulator)
...
// HELPER: Tentukan Rank Seseorang (Untuk Statistik Grid)
...
// FUNGSI HITUNG STATISTIK VIP GRID
...
// LOGIKA STATUS SAYA (UTAMA) - PERBAIKAN BUG TARGET
function calculateMyRank(total, direct) {
    ...
    // 1. Cek apakah sudah masuk VIP 2 - 9?
    ...
    if (currentRank) {
        ...
        if (currentIndex > 0) { // Masih ada rank di atasnya
            let nextRank = ranks[currentIndex - 1]; // Rank di atasnya (index lebih kecil)
            
            gap = nextRank.min - total;
