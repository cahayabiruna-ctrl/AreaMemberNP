...
async function loadData() {
    ...
    } catch (e) { console.error("Err Data:", e); }
}
...
// Total Group = 1 (Diri Sendiri) + Total Downline
...
// LOGIKA RANK UNTUK STATISTIK (HELPER)
function getRankLevel(uid) {
    // Rank Orang Lain dihitung berdasarkan Total Group MEREKA
    const total = getTotalGroupCount(uid);
    // Dan jumlah Direct MEREKA
    ...
    if (direct >= 5) return 1; // Syarat VIP 1
    return 0; // Member
}
...
function countVipStats(downlines) {
    ...
    downlines.forEach(m => {
        let rank = getRankLevel(m.uid); // Cek Rank anggota ini
        if (rank >= 1 && rank <= 9) {
            counts[rank]++;
        }
    });
...
function calculateMyRank(total, direct) {
    ...
    if (currentRank) {
        ...
        if (idx > 0) {
            ...
            gap = 0; descHtml = "Maksimal"; nextName="Max";
        }
    } else {
        if (direct >= 5) {
            rankName = "V.I.P 1";
